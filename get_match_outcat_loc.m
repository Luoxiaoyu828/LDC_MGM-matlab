function [match_num, match_record,F1,find_outcat_points,simulate_outcat_points]=get_match_outcat_loc(simulate_outcat_path,find_outcat_points, dc)
% 计算仿真核与聚类中心的匹配情况
...
if ~isstr(find_outcat_points)
    find_outcat_points=find_outcat_points(:,4:6);
end
...
while 1
    ...
    [index_0, index_1] = find(distance == distance_min);
    match_record=[match_record; [index_0, index_1, distance_min]];
    distance(index_0, :) = ones(1, size(distance,2)) .* max_d;
    distance(:, index_1) = ones(size(distance,1),1) .* max_d;
    match_num = match_num + 1;
    if min(distance(:)) > dc || match_num >= size(simulate_outcat_points,1)
        break
    end
end
