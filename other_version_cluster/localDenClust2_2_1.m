function [outcat, out, mask, Gradient] = localDenClust2_2_1(data,para,is_plot,get_out_mask,local_max_index)
%   INPUT:
...
if length(size(data))<=2
    ...
    xx=[p_j(:), p_i(:)];
    [NClust,centInd,clustInd,~,mask,out,Gradient]=densityCluster_2d(data,xx,para,is_plot,get_out_mask,local_max_index);
end
...
%% densityCluster 2D
function [NCLUST_,centInd,clustInd,cluster_info,mask,out,Gradient]=densityCluster_2d(data,xx,para,is_plot,get_out_mask,local_max_index)
%   SEE the following paper published in *SCIENCE* for more details:
%       Alex Rodriguez & Alessandro Laio: Clustering by fast search and find of density peaks,
%       Science 344, 1492 (2014); DOI: 10.1126/science.1242072.
%       做了部分修改,2020/12/08,vastlxy@163.com
...
% clustInd=-1 表示该点不是类的中心点，属于其他点，等待被分配到某个类中去
% 类的中心点的梯度Gradient被指定为-1
disp(min(IndNearNeigh))
for i=1:ND
    ordrho_i=rho_Ind(i);
    if clustInd(ordrho_i)==-1 && IndNearNeigh(ordrho_i)>0 % not centroid
