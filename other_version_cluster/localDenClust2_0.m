function [outcat, out, mask, Gradient] = localDenClust2_0(data,para,is_plot,get_out_mask)
%   INPUT:
    ...
    if rho_ii >=rms
        ...
        for j_=1:size(bt,1)
            ...
            if dist_i_j <= delta(ordrho_ii) && gradient>=Gradient(ordrho_ii)
                ...
                Gravitation(ordrho_ii) = rho_ii * rho_jj / dist_i_j;
            end
        end
        if delta(ordrho_ii)==maxd
            ...
            for jj=1:ii-1
                ...
                if dist_i_j<=delta(ordrho_ii) 
                    ...
                    Gravitation(ordrho_ii) = rho_ii * rho_jj / (dist_i_j^2);
                end
            end
        end
    else
...
disp(detla_sorted(2))
Gravitation(rho_Ind(1)) = rho_sorted(1) * rho_sorted(2) / (detla_sorted(2)^2);
disp(rho_sorted(1))
disp(rho_sorted(2))
