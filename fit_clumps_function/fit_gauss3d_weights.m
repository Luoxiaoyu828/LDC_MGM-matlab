function [beta,MSE,R,Sum] = fit_gauss3d_weights(x_tem,y_r,beta0,weights)
% inputs:
% x_tem: 坐标
% y_r: 强度值
% beta0: 拟合的初始值
...
% 不带旋转的3维高斯分布
gauss_3d = @(a, x) ( a(1) * exp( -(((x(:,1)-a(2)).^2)./(2*(a(3))^2) + ((x(:,2)-a(4)).^2)./(2*(a(5))^2)  ...
    + ((x(:,3)-a(6)).^2)./(2*(a(7))^2) ) ) );
...
yr_weigths = weights.^2/max(weights.^2);
